...
namespace MBKM.Presentation.Areas.Portal.Controllers
{
    ...
    public class PendaftaranMataKuliahController : Controller
    {
        ...
        public PendaftaranMataKuliahController(
            IApprovalPendaftaranService approvalPendaftaranService, 
            ICPLMatakuliahService cplMatakuliahService, 
            ICPLMKPendaftaranService cplmkPendaftaranService, 
            IPendaftaranMataKuliahService pendaftaranMataKuliahService, 
            IInformasiPertukaranService informasiPertukaranService, 
            IPerjanjianKerjasamaService perjanjianKerjasamaService, 
            IJenisKerjasamaModelService jenisKerjasamaService, 
            IPendaftaranMataKuliahService pmkService, 
            IMahasiswaService mahasiswaService, 
            IJadwalKuliahService jkService)
        {
    ...
    public ActionResult InsertInformasiPertukaran(InformasiPertukaran informasiPertukaran) {
      try {
        ...
        // tambahin if lg di sini buat cek data mhs sdh ada ato blm
        //var ipstrm = _informasiPertukaranService.Find(_ => _.STRM == (int)_pmkService.getOngoingSemester(GetMahasiswaById(long.Parse(Session["idMahasiswa"] as string)).JenjangStudi).ID);
        //var ipstrm = _informasiPertukaranService.Find(_ => _.MahasiswaID == id && _.STRM == (int)_pmkService.getOngoingSemester("S1").ID).FirstOrDefault();
        Int64 strmmhs = (int)_pmkService.getOngoingSemester(GetMahasiswaById(long.Parse(Session["idMahasiswa"] as string)).JenjangStudi).ID;
        var ipstrm = _informasiPertukaranService.Find(_ => _.MahasiswaID == id && _.STRM == strmmhs).FirstOrDefault();
        // 2 baris pasangan ini bisa
        // Int64 ipstrm = (int)_pmkService.getOngoingSemester(GetMahasiswaById(long.Parse(Session["idMahasiswa"] as string)).JenjangStudi).ID;
        // if (ipstrm != 2010) 
        ...
        if (ipstrm == null) 
        {
          ...
          ipstrm.JenisKerjasama = informasiPertukaran.JenisKerjasama;
          ipstrm.NoKerjasama = informasiPertukaran.NoKerjasama;
          ipstrm.JudulAktivitas = informasiPertukaran.JudulAktivitas;
          ipstrm.LokasiTugas = informasiPertukaran.LokasiTugas;
          ipstrm.TanggalSK = informasiPertukaran.TanggalSK;
          ipstrm.NoSK = informasiPertukaran.NoSK;
          ipstrm.UpdatedBy = HttpContext.Session["nama"].ToString();
          ipstrm.UpdatedDate = DateTime.Now;
          try {
            _informasiPertukaranService.Save(ipstrm);
          }
          catch (Exception) {
            ipstrm.TanggalSK = null;
            _informasiPertukaranService.Save(ipstrm);
          }
        }
        ...
        public ActionResult GetMataKuliahByProdi(string fakultas, string prodi, string lokasi, int strm, string matkul)
        {
            ...
            var checkMahasiswa = GetMahasiswaById(long.Parse(Session["idMahasiswa"] as string)).NIM;

            if (checkMahasiswa.Substring(4, 4) == "9001")
            {
                result = _jkService.Find(
                        jk =>
                        jk.NamaFakultas == fakultas &&
                        jk.NamaProdi == prodi &&
                        jk.Lokasi == lokasi &&
                        jk.STRM == strm &&
                        jk.FlagOpenEksternal == true &&
                        jk.FakultasID != 99 &&
                        jk.FakultasID != 00 &&
                        jk.FakultasID != 88)
            ...
            else
            {
                if (matkul != null && matkul.Length > 0)
                {
                    result = _jkService.Find(
                        ...
                        jk.STRM == strm &&
                        jk.FlagOpen == true &&
                        jk.KodeMataKuliah + " - " + jk.NamaMataKuliah == matkul &&
                        jk.FakultasID != 99 &&
                        jk.FakultasID != 00 &&
                        jk.FakultasID != 88)
                            .GroupBy(g => new { g.ClassSection, g.JamMasuk, g.JamSelesai, g.SKS, g.KodeMataKuliah, g.MataKuliahID, g.NamaMataKuliah, g.STRM })
